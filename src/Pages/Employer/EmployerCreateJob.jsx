...
const jobSchema = z.object({
  ...
  summary: z.string().max(512, 'Summary is too long').optional().or(z.literal('')),
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <header>
          ...
          <p className="mt-1 text-sm text-slate-500">Define the primary details candidates will see for this job.</p>
        </header>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <header>
          ...
          <p className="mt-1 text-sm text-slate-500">Share salary context and control if/when this job is live.</p>
        </header>
        <div className="mt-6 grid gap-6 md:grid-cols-2">
          <div>
      ...
      <div className="flex items-center justify-end gap-3">
        ...
        >
          {mutation.isLoading ? 'Publishing…' : 'Create job'}
        </button>
      </div>
    </form>
  );
