...
const EmployeeFavorites = () => {
  ...
  if (!favorites.length) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-slate-900 px-6 py-12 text-center text-sm text-slate-500">
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      {favorites.map((favorite) => (
        <article key={favorite.favorite.id} className="rounded-2xl border border-slate-200 bg-slate-900 p-6 shadow-sm">
          ...
          <div className="mt-4 flex justify-end gap-3">
            <button
              ...
              className="inline-flex items-center justify-center rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-slate-900 transition hover:bg-indigo-500"
            >
              View details
            </button>
          </div>
